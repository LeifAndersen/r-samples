...
setGeneric("mkGraph", function(n,e) standardGeneric("mkGraph"))
setMethod("mkGraph",
          c("numeric","numeric"),
          function(n,e) {
              g <- new("Graph")
              for(i in 1:n) {
                  g <- add.node(g, paste(sample(c(0:9, letters, LETTERS),
                                               32, replace=TRUE),
                                         collapse=""))
              }
              for(i in 1:e) {
...
kruskal(g)
Rprof(NULL)
summaryRprof(filename="output.out")
